# public-news-site Specification
...
The system SHALL render a home page that lists each active category along with the latest published news items in that category (default 5 per category, ordered by `publishedAt` DESC).
...
The system SHALL render a "Most Viewed Today" section on the home page listing the top 5 news items by today's view count, ordered DESC.

#### Scenario: Most viewed today is rendered
- **WHEN** a visitor opens `/`
- **THEN** a "Most Viewed Today" widget displays up to 5 news items with rank, title, and view count, linking to each detail page
...
The system SHALL provide a category page at `/category/{slug}` that loads news items 10 at a time via infinite scroll.
...
The system SHALL render an article detail page at `/news/{slug}` that displays the full content and provides links to the next-newer and next-older articles in the same category.
